...
def weave(
    ...
    known_words = []
    ai_prompt = f"The aim is to create a diglot weave based on a list of known words, then to slowly introduce new words (similar to Prismatext). An English text has been provided. Also a list of known {target_lang} words (as lemmas) has been provided. Replace words or phrases from the text with their {target_lang} equivalents. A literal or word-for word translation will not succeed, so when you notice that it is appropriate to add a {target_lang} word, ALTER THE SENTENCE AS NEEDED to make it grammatically correct (or as close as possible) in both languages (such as adjectives coming after nouns in some European languages). FOR EXAMPLE, multiple words may be replaced by a single word in the target language or vice versa (e.g. in Russian 'a car' becomes 'машина', not 'a машина', 'have been' becomes 'были', 'to go' becomes 'идти', etc. All of these little grammatical rules that don't translate literally between the languages). If a word/lemma is known, it should appear in all appropriate instances, with correct inflection, conjugation, gender, and any other grammatical rules not found in English. Gradually (less than 1% of words) introduce new {target_lang} words (EASIER, EVERYDAY WORDS COME FIRST). Ensure that grammar, punctuation and capitalisation are consistent with rules in both English and {target_lang}. If/when a clause contains mostly words that are known, it should be structured like a {target_lang} sentence (in terms of grammar, word order etc.) rather than retaining any of the original English structure. Return 2 objects. 1. The new text (which will be a hybrid of English and Russian). Retain input formatting and include a double newline in between paragraphs (if not already present). When introducing a {target_lang} word, it MUST be in the format {{{target_lang}Word|Lemma|Original Word(s)}} with no additional emphasis or all-caps for the Russian word and the lemma in lower-case. Beyond this, no additional formatting. No emphasising the {target_lang} words with asterisks or all caps. And 2. Return the list of newly added {target_lang} lemmas."
    output_text = ""
    output_words = []
